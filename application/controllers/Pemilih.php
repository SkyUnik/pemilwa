...
class Pemilih extends CI_Controller
{
	...
	public function tambah_pemilih()
	{
		...
		// $this->form_validation->set_rules('kls[]', 'kls', 'required');
		...
		if ($this->form_validation->run() == TRUE) {
			...
			//atur nilai kls
			$dap = null;
			$post_kls = $this->input->post('kls');
			foreach ($post_kls as $d) {
				if ($dap == null) {
					$dap = $d;
				} else {
					$dap = $dap . ',' . $d;
	...
	public function upload_exel_pemiilih()
	{
		...
		//jika berhasil upload exel
			...
			for ($i = 2; $i <= $totalrows; $i++) {
				...
				$kls = $objWorksheet->getCellByColumnAndRow(2, $i)->getValue();
				$data_user = array('id' => $id, 'nama' => $nama, 'kls' => $kls);
	...
	public function update_pemilih($ids)
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$dpl = array(null => '-- Pilih kls yag sesuai --', );
			foreach ($query as $j) {
				$dpl[$j->kode_kls] = $j->kls;
			}
			;
