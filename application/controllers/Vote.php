...
class Vote extends CI_Controller
{
	...
	public function login()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			//ambil data pemilih
			$voter_data = $this->Pemilwa_model->get_akun($this->input->post('nis'), 'pemilih');
			//jika nim tidak ditemukan
			if ($voter_data == FALSE) {
				$this->session->set_flashdata('error', 'NIS tidak terdaftar.');
				redirect('vote/login', 'refresh');
			}
			...
			switch ($status) {
				...
				case '0':
					...
					if ($this->Pemilwa_model->pengaturan('verifikasi_voter')) {
						$this->session->set_flashdata('error', 'NIS belum diverifikasi panitia TI, silakan hubungi panitia TI terlebih dahulu.');
						redirect('vote/login', 'refresh');
				...
				case '2':
					$this->session->set_flashdata('error', 'NIS telah memberikan hak suara.');
					redirect('vote/login', 'refresh');
					break;
				//jika diblokir
				case '3':
					$this->session->set_flashdata('error', 'NIS diblokir.');
					redirect('vote/login', 'refresh');
					break;
				default:
					$this->session->set_flashdata('error', 'NIS tidak terdaftar di daftar pemilih tetap.');
					redirect('vote/login', 'refresh');
					break;
			}
			if ($status == '1') {
