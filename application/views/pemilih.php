  ...
  <h2>
    ...
    <table class="table table-hover table-striped">
      <thead>
        <?php if (is_null($search_mode)): ?>
          <tr>
            <th scope="col">NIS <a href="<?php echo base_url('pemilih/show/id') ?>">^</a></th>
            <th scope="col">Nama <a href="<?php echo base_url('pemilih/show/nama') ?>">^</a></th>
            <th scope="col">Kelas <a href="<?php echo base_url('pemilih/show/dap') ?>">^</a></th>
      ...
      <tbody>
        ...
        foreach ($data_pemilih as $pemilih) {
          ...
          $dap = explode(',', $pemilih->kls);
          $view_kls = null;
          foreach ($dap as $d) {
            $query = $this->db->select('kls')->get_where('jabatan', array('kode_kls' => $d))->row();
            if ($view_kls == null) {
              $view_kls = $query->kls;
            } else {
              $view_kls = $view_kls . ', ' . $query->kls;
            }
          }
