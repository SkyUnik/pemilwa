<div class="container">
	...
	?>
	<p class="alert alert-warning alert-dismissable"><a href="#" type="button" class="close" data-dismiss="alert"
			aria-label="Close"><span aria-hidden="true">&times;</span></a><strong>
			<?php echo $id_pemilih; ?>
			<?php echo $nama_pemilih->nama; ?>
		</strong>
		<br>Pilih satu kandidat pada setiap
		jabatan, klik tombol <b>next</b> untuk lanjut ke jabatan selanjutnya.
		<br>
		<a style="margin-top: 1rem;" class="btn btn-sm btn-danger"
			href="<?php echo base_url('vote/logout') ?>">Logout</a>
	</p>
	...
	<?php
	$dapil_pemilih = $this->db->select('kls')->get_where('pemilih', array('id' => $this->session->userdata('voter_id')))->row();
	$dap = explode(',', $dapil_pemilih->kls);

	foreach ($dap as $d) {
		$jabatan = $this->db->get_where('jabatan', array('kode_kls' => $d))->result();
